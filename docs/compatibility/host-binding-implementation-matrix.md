...
| Semantic Grant request | `OfflineSemanticGrantRequest` checks the closed selected request, exact manifest/delegate bindings and `grant-request/v1` hashing view; it derives exposure for later presentation but rejects server outputs | Bind the request to a concrete host-owned Consent Preview and two distinct decisions only after authenticated host adapters exist | Expectations are caller-supplied integrity facts, not authentication. OAuth `type`, subject, Grant fields, credential binding, exposure and raw credentials reject; the value is not an issuance DTO or approved-record capability. |
...
3. **Semantic request, Grant values and projection (ADP-05) — bounded offline
   slices completed.** `OfflineSemanticGrantRequest` fixes the selected request
   hashing view and rejects server outputs. `OfflineSelectedGrant` implements
   the selected authoritative hashing view, tuple/method/identity/exposure
   validation and derivation against a retained prepared manifest. Trusted
   inputs and tests do not qualify the external verifier, consent UI, issuer
   state or runtime mediation.
