...
export class OfflineSemanticGrantRequest {
  ...
  prepare(): PreparedOfflineSemanticGrantRequest {
    ...
    const requestSnapshot = this.#snapshotRequest();
    const request = this.#parseRequest(requestSnapshot);
    ...
    const computed = grantRequestHash(requestSnapshot);
  ...
  #snapshotRequest(): JsonDocument {
    const parsed = this.#parseRequest(this.#request);
    try {
      const source = JSON.stringify(parsed);
      if (typeof source !== 'string')
        throw new Error('grant_request_incompatible');
      return new JsonDocument(source);
    } catch {
      throw new Error('grant_request_incompatible');
    }
  }

  #parseRequest(document: JsonDocument): JsonRecord {
    try {
