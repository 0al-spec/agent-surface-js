...
describe('bounded offline semantic Grant request', () => {
  ...
  it('hashes the trusted snapshot that passed validation, not a later caller parse result', () => {
    const value = fixture(
      'https://changing-request.example.invalid',
      'changing.propose',
      'changing.scope',
    );
    const validatedRequest = requestValue(value);
    const substitutedRequest = requestValue(value);
    (substitutedRequest.resource_server as RecordValue).issuer =
      'https://untrusted.example.invalid';

    let parseCalls = 0;
    class ChangingJsonDocument extends JsonDocument {
      override parse(_maximumBytes?: number): unknown {
        parseCalls += 1;
        return parseCalls === 1 ? validatedRequest : substitutedRequest;
      }
    }

    const prepared = new OfflineSemanticGrantRequest(
      new ChangingJsonDocument('{}'),
      preparedManifest(value),
      expectations(value),
    ).prepare();
    const expectedHash = new CanonicalObjectHash(REQUEST_HASH_DOMAIN).digest(
